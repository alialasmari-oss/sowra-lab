...
const BAD_PATTERNS = [
  ...
  /* __SPB وwindow.banner من عهد ما قبل الوحدات، ولا وجود لهما.
     وكانا يُقرآن داخل سلسلةٍ نصية: ['banner','__SPB'].forEach(k =>
     if(window[k]) …) — فالشرط كاذبٌ أبداً والجسم لا يعمل، ومفتاح
     «أضواء الديرة» يحتاج ضغطتين. والنمط العام أعلاه يستثني ما
     سبقته علامة اقتباس، فلا يراه — فله نمطه الخاص بلا استثناء.
     ولا نفحص window[k] عامةً: تلك وظيفة core/bridge.js نفسها. */
  [/__SPB/,                              'متغير عام ميت'],
];
const badHits = [];
for(const f of files){
